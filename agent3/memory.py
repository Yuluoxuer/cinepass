"""agent3 对话历史存储：``agent_messages`` 表（用户输入 + Agent 输出）。

短期记忆（图的运行状态，含 history）由 LangGraph PostgresSaver（``agent3.checkpoint``）
持久化；本模块把每轮对话（user 输入 + assistant 回复）落到独立的 ``agent_messages`` 表，
供会话历史端点 ``GET /agent3/sessions/{id}/messages`` 读取与分页。
"""
...
async def load_history(session_id: str, limit: int | None = None) -> list[dict[str, str]]:
    ...
    limit 缺省取 ``settings.memory_window * 2``（默认 20 轮 → 40 条）。
    """
    if limit is None:
