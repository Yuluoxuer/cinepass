...
from agent3.http import backend_url, delete, get, post
from agent3.request_context import has_authorization
...
async def get_seat_map(show_id: str) -> str:
    """查看某场次的座位图。show_id=场次ID。返回原始 JSON 数据。"""
    try:
        data = _unwrap(await get(backend_url(f"/shows/{show_id}/seat-map"), timeout=0.8))
...
async def recommend_seats(
    ...
    try:
        data = _unwrap(await post(backend_url("/reco/seats"), json=body, timeout=1.2))
