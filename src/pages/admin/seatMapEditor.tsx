...
const SeatMapEditorPage: React.FC = () => {
  ...
  const [cinemaId, setCinemaId] = useState(queryCinemaId);
  ...
  useEffect(() => {
    setCinemaId(queryCinemaId);
    void catalogApi.listCinemas({ sort: 'price', page: 1, size: 50 })
      .then((result) => setCinemas(result.items))
      .catch(() => setCinemas([]));
  }, [queryCinemaId]);
  ...
  const paint = (row: number, col: number) => setGrid((previous) => {
  ...
  const save = async () => {
    ...
    try {
      ...
      history.replace(`/admin/cinemas/${created.cinemaId || cinemaId}/halls?seatMapId=${encodeURIComponent(created.seatMapId)}`);
    } catch {
      // 请求层已处理。
    }
  };

  if (seatMapId && seatMapId !== 'new') {
    return <Alert
      type="info"
      showIcon
      message="当前真实接口暂不支持查看或编辑座位图"
      description="请返回座位图入口新建座位图；已创建的座位图可通过影厅管理页中的座位图 ID 关联。"
      action={<Button size="small" onClick={() => history.push('/admin/seat-maps')}>返回座位图</Button>}
    />;
  }

  return <div>
    <Space wrap style={{ marginBottom: 16 }}>
      <Select placeholder="所属影院" style={{ width: 220 }} value={cinemaId || undefined} disabled={!!queryCinemaId} options={cinemas.map((cinema) => ({ value: cinema.cinemaId, label: cinema.name }))} onChange={setCinemaId} />
      <span>行</span><InputNumber min={1} max={30} value={rows} onChange={(value) => { const nextRows = value || 1; setRows(nextRows); resizeGrid(nextRows, cols); }} />
      <span>列</span><InputNumber min={1} max={40} value={cols} onChange={(value) => { const nextCols = value || 1; setCols(nextCols); resizeGrid(rows, nextCols); }} />
      <Button onClick={generate}>生成画布</Button><Input style={{ width: 120 }} value={screenLabel} onChange={(event) => setScreenLabel(event.target.value)} placeholder="银幕" />
      <Input style={{ width: 160 }} value={id} maxLength={32} onChange={(event) => setId(event.target.value)} placeholder="座位图 ID（可选）" /><Button type="primary" onClick={save}>创建座位图</Button>
    </Space>
