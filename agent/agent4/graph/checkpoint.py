"""agent4 短期记忆：LangGraph PostgresSaver（checkpointer）生命周期管理。

与 ``agent/langgraph/checkpoint.py`` 同构，但使用 agent4 自包含配置
（``agent4.config.get_settings().postgres_uri``）。未配置 Postgres 时返回 ``None``，
图以无状态模式运行。
"""
